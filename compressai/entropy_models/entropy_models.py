...
#GMM - ywz gmm_full:每个像素点都有一组weights
class GaussianMixtureConditional_gf(EntropyModel):
    r"""Gaussian conditional layer, introduced by J. Ballé, D. Minnen, S. Singh,
    S. J. Hwang, N. Johnston, in `"Variational image compression with a scale
    hyperprior" <https://arxiv.org/abs/1802.01436>`_.
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def _likelihood(self, inputs, scales, means=None,weights=None):
        ...
        for k in range(self.K):
            ...
            # print(values.shape) #([1, 192, 32, 32])
            # print(upper.shape) #([1, 192, 32, 32])
            # raise ValueError("stop valueshape")
            ...
            # raise ValueError("stop likelihood shape")
            # if likelihood==None:
            #     likelihood = (upper - lower)*1/self.K #指定对应的M个通道
            # else:
            #     likelihood += (upper - lower)*1/self.K #指定对应的M个通道

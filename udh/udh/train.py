...
class HomographyModel(pl.LightningModule):
    def __init__(self, myargs):
        ...
        self.myargs = myargs
        # print(self.myargs)
        self.model = Net(patch_size=self.myargs.patchsize)
    ...
    def configure_optimizers(self):
        return torch.optim.Adam(self.model.parameters(), lr=self.myargs.learning_rate)

    def train_dataloader(self):
        train_set = SyntheticDataset(self.myargs.train_path, rho=self.myargs.rho,pic_size=self.myargs.picsize,patch_size=self.myargs.patchsize)
        return DataLoader(
            train_set,
            num_workers=4,
            batch_size=self.myargs.batch_size,
            shuffle=True,
            collate_fn=safe_collate,
        )

    def val_dataloader(self):
        val_set = SyntheticDataset(self.myargs.valid_path, rho=self.myargs.rho,pic_size=self.myargs.picsize,patch_size=self.myargs.patchsize)
        return DataLoader(
            val_set,
            num_workers=4,
            batch_size=self.myargs.batch_size,
...
def main(args):
    ...
    if args.resume == "":
        model = HomographyModel(myargs=args)

...
class HSIC(CompressionModel):
    ...
    def forward(self,x1,x2,h_matrix):
        ...
        x2_warp = kornia.warp_perspective(x2, h_inv, (x2.size()[-2], x2.size()[-1]))
        #定义结构
        y1 = self.encoder1(x2_warp,x1)
        z1 = self._h_a1(y1)
        #print(z1.device)
        ...
        x1_hat_warp = kornia.warp_perspective(x1_hat_temp, h_matrix, (x1_hat_temp.size()[-2],x1_hat_temp.size()[-1]))
        ...
        x2_hat = self.decodercat1(x2_hat_temp, x1_hat_warp)

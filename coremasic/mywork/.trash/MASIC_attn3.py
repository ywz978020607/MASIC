...
class Encoder1(nn.Module):
    def __init__(self, N, M, **kwargs):
        ...
        self.g_a_gdn2 = GDN(N)
        self.attn1 = ResidualBlock(N, N)
        self.g_a_conv3 = conv(N, N)
        self.g_a_gdn3 = GDN(N)
        self.g_a_conv4 = conv(N, M)
        self.attn2 = ResidualBlock(M, M)
    ...
    def forward(self, x):
        ...
        self.g_a_g2 = self.attn1(self.g_a_gdn2(self.g_a_c2))
        self.g_a_c3 = self.g_a_conv3(self.g_a_g2)  # Tensor
        self.g_a_g3 = self.g_a_gdn3(self.g_a_c3)
        self.g_a_c4 = self.attn2(self.g_a_conv4(self.g_a_g3))  # Tensor
        self.y = self.g_a_c4
...
class HSIC(CompressionModel):
    ...
    def forward(self,x1,x2,h_matrix):
        ...
        # x1_warp = kornia.warp_perspective(x1_hat, h_matrix, (x1.size()[-2],x1.size()[-1]))
        # y2 = self.encoder2(x1_warp,x2)
        y2 = self.encoder2(x1_warp_aftercodec,x2)
        ##end encoder
...
class Independent_EN(nn.Module):
    def __init__(self):
        ...
        self.attn_add1 = ResidualBlock(96, 96)
        self.attn_add2 = ResidualBlock(96, 96)

        self.attn_final1 = ResidualBlock(3, 3)
        self.attn_final2 = ResidualBlock(3, 3)
        # Enhancement
    ...
    def forward(self, x1_hat, x2_hat, h_matrix):
        ...
        #attn-add #注: 不建议同名反复覆盖  不便于微改模型重新load
        out1 = self.attn_add1(out1)
        out2 = self.attn_add2(out2)
        ...
        x1_hat2_attn = self.attn_final1(x1_hat2)
        x2_hat2_attn = self.attn_final2(x2_hat2)

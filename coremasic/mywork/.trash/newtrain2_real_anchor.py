...
#python newtrain2_real_anchor.py -d "/home/ywz/database/aftercut512"  --seed 0 --cuda 0 --patch-size 512 512 --batch-size 1 --test-batch-size 1  --save --lambda 0.01
...
def main(argv):
    ...
    net = HSIC(N=192,M=192,K=5) #Residual -- M=N
    ...
    if os.path.exists("checkpoint_best_loss.pth.tar"):
        ...
        # pretrained_dict = {k: v for k, v in model['state_dict'].items() if (k in new_dict and "encoder" not in str(k) and "decoder" not in str(k))}#filter out unnecessary keys

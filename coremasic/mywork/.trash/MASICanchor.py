...
class Encoder1(nn.Module):
    def __init__(self, N, M, **kwargs):
        super().__init__()
        self.g_a_conv1 = ResidualBlockWithStride(3, N, stride=2)#,conv(3, N)
        self.g_a_gdn1 = ResidualBlock(N, N)#,GDN(N)
        self.g_a_conv2 =  ResidualBlockWithStride(N, N, stride=2)#,conv(N, N)
        self.g_a_gdn2 = ResidualBlock(N, N)#,GDN(N)
        self.g_a_conv3 = ResidualBlockWithStride(N, N, stride=2)#,conv(N, N)
        self.g_a_gdn3 = ResidualBlock(N, N)#,GDN(N)
        self.g_a_conv4 = conv3x3(N, M, stride=2)#conv(N, M)
...
class Decoder1(nn.Module):
    def __init__(self, N, M, **kwargs):
        super().__init__()
        self.g_s_conv1 = ResidualBlock(M, N)#,deconv(M, N)
        self.g_s_gdn1 = ResidualBlockUpsample(N, N, 2)#,GDN(N, inverse=True)
        self.g_s_conv2 = ResidualBlock(N, N)#,deconv(N, N)
        self.g_s_gdn2 = ResidualBlockUpsample(N, N, 2)#,GDN(N, inverse=True)
        self.g_s_conv3 = ResidualBlock(N, N)#,deconv(N, N)
        self.g_s_gdn3 = ResidualBlockUpsample(N, N, 2)#,GDN(N, inverse=True)
        self.g_s_conv4_0 = ResidualBlock(N, N)#,deconv(N, 3)
        self.g_s_conv4 = subpel_conv3x3(N, 3, 2)#,deconv(N, 3)
    ...
    def forward(self, y_hat):
        ...
        #change
        self.g_s_c4_0 = self.g_s_conv4_0(self.g_s_g3)  # Tensor
        self.g_s_c4 = self.g_s_conv4(self.g_s_c4_0)  # Tensor
        #
    ...
    def compress(self,x1,x2,h_matrix,output_name,output_path="",device="cpu"):
        ...
        for h_idx in range(y2_hat_cpu_np.shape[2]):
            for w_idx in range(y2_hat_cpu_np.shape[3]):
                ...
                # print(p2.shape)
                # print(ctx_p2.shape)
                # print(y1_hat_warpf2.shape)
                gmm2_jq = self._h_s2_same_resolution(torch.cat((p2 * mask_weights[:, 0:1, h_idx:h_idx+1,w_idx:w_idx+1],
                                                                ctx_p2 * mask_weights[:, 1:2, h_idx:h_idx+1,w_idx:w_idx+1],
                                                                y1_hat_warpf2[0:1,:,h_idx:h_idx+1,w_idx:w_idx+1] * mask_weights[:, 2:3, h_idx:h_idx+1,w_idx:w_idx+1]),
                                                               dim=1))  # 通道数 4M
    ...
    def decompress(self,x1,x2,h_matrix,output_name,output_path="",device="cpu"):
        ...
        for h_idx in range(y_shape[0]):
            for w_idx in range(y_shape[1]):
                ...
                gmm2_jq = self._h_s2_same_resolution(torch.cat((p2 * mask_weights[:, 0:1, h_idx:h_idx + 1, w_idx:w_idx + 1],
                               ctx_p2 * mask_weights[:, 1:2, h_idx:h_idx + 1, w_idx:w_idx + 1],
                               y1_hat_warpf2[0:1, :, h_idx:h_idx + 1, w_idx:w_idx + 1] * mask_weights[:, 2:3,h_idx:h_idx + 1,w_idx:w_idx + 1]),dim=1))  # 通道数 4M

...
# test2_save_debug:产出h矩阵计算保存相应的视差值和匹配图

#python test2_save_debug.py -d "/home/yangwenzhe/database/aftercut512"  --seed 0 --cuda 0 --patch-size 512 512 --batch-size 1 --test-batch-size 1
#cpu版
#python test2.py -d "/home/sharklet/database/aftercut512"  --seed 0  --patch-size 512 512 --batch-size 1 --test-batch-size 1

#net defination
from newnet1_jg_codec_gf_black4 import *
...
def test_epoch(epoch, test_dataloader, model, criterion):
    ...
    with torch.no_grad():
        for d in test_dataloader:
            ...
            d1_warp = kornia.warp_perspective(d1, h_matrix, (d1.size()[-2], d1.size()[-1]))
            mask_R,mask_L = mask(d1,h_matrix)
            ...
            print(allbpp)
            raise ValueError("stop for bpp")
            # out_criterion = criterion(out_net, d1,d2)
            ##########
            # # 保存mask1照片
            # print(out_net['x1_mask_R'])
            # print(out_net['x1_mask_R'].shape)
            save_pic(out_net['x1_mask_R'], osp.join(right_save_path,"x1_mask_R_"+str(d[3]).split("'")[1]))
            save_pic(out_net['x1_mask_L'], osp.join(left_save_path, "x1_mask_L_" + str(d[3]).split("'")[1]))
            save_pic(out_net['x1_hat'], osp.join(left_save_path,"x1_hat_"+str(d[3]).split("'")[1]))
            save_pic(out_net['x2_hat'], osp.join(right_save_path,"x2_hat_"+str(d[3]).split("'")[1]))
            # raise ValueError("stop")
            # ############
            ...
            # bpp_val = out_criterion['bpp_loss']
            # bpp1_val = out_criterion['bpp1']
            # bpp2_val = out_criterion['bpp2']
            #
            # print_context = (str(d[3]).split("'")[1]+
            #       f'\tPSNR (dB): {(psnr1_val + psnr2_val) / 2:.3f} |'  # 平均一张图的PSNR
            #       f'\tBpp loss: {bpp_val / 2:.4f} |'
            #       f'\tBpp1 loss: {bpp1_val:.4f} |'
            #       f'\tBpp2 loss: {bpp2_val:.4f} |'
            #       f'\tMS-SSIM: {ssim_val:.4f} |'  # 已除2，相加时候便除了2
            #       f'\tPSNR1: {psnr1_val:.3f} |'
            #       f'\tPSNR2: {psnr2_val:.3f}\n')
            #

            count_R = float(torch.sum(mask_R!=0)) / float((d1.size()[-2]* d1.size()[-1]))
            count_L = float(torch.sum(mask_L != 0)) / float((d1.size()[-2] * d1.size()[-1]))

            print_context = (str(d[3]).split("'")[1]+",count_R:%.2f"%count_R+",count_L:%.2f"%count_L  +  "  \n")
            ...
            save_pic(d1_warp, osp.join(left_save_path, str(d[3]).split("'")[1]))
            save_pic(d2, osp.join(right_save_path, str(d[3]).split("'")[1]))

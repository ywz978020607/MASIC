...
#python newtrain6.py -d "/home/sharklet/database/aftercut512"  --seed 0 --cuda 0 --patch-size 512 512 --batch-size 1 --test-batch-size 1  --save --lambda
...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target1,target2):
        ...
        scale_factor = (0.01 ** ((10 * torch.log10(self.mse(output['x1_hat'], target1)) - 10 * torch.log10(self.mse(output['x2_hat'], target2)))))  # torch.abs  #前-  后+  右目质量高时 factor<1
        out['mse_loss'] = self.mse(output['x1_hat'], target1) + self.mse(output['x2_hat'], target2)  #*scale_factor        #end to end
        out['loss'] = self.lmbda * 255**2 * out['mse_loss'] #+ 3*(10 * torch.log10(self.mse(output['x1_hat'], target1)) - 10 * torch.log10(self.mse(output['x2_hat'], target2))) #+ out['bpp_loss']

...
# python newtrain2_for_jg_gf_black5.py -d "/home/yangwenzhe/database/aftercut512"  --seed 0 --cuda 0 --patch-size 512 512 --batch-size 1 --test-batch-size 1  --save --lambda 0.01 -e 800
# python newtrain2_for_jg_gf_black4.py -d "/home/yangwenzhe/database/aftercut512" -da "/home/yangwenzhe/database/flickr"  --seed 0 --cuda 2 --patch-size 512 512 --test-patch-size 960 640 --batch-size 1 --test-batch-size 1   --save --lambda 0.025 -e 800
#
#net defination
from MASIC_3x3 import *
...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target1,target2):
        ...
        # scale_factor = (2**(self.ELU(10*torch.log10(self.mse(output['x1_hat'], target1) ) - 10*torch.log10(self.mse(output['x2_hat'], target2) ) )))
        # scale_factor = (0.9 ** ((10 * torch.log10(self.mse(output['x1_hat'], target1)) - 10 * torch.log10(
        #     self.mse(output['x2_hat'], target2))))) #torch.abs  #前-  后+  右目质量高时 factor<1
        out['mse_loss'] = self.mse(output['x1_hat'], target1) +  self.mse(output['x2_hat'], target2) #*scale_factor        #end to end
        # out['loss'] = self.lmbda * 255**2 * out['mse_loss'] + out['bpp_loss']
        out['loss'] = self.lmbda * 255 ** 2 * out['mse_loss'] + out['bpp_loss'] #0.01    0.001-0.05  ->[65,3000]  -- yr:1024  [256,2048] ->ssim: [8,64]  : lambda*mse + bpp -> lambda*ssim + bpp
...
def train_epoch(epoch, train_dataloader, model, criterion, optimizer,
    ...
    for i, d in enumerate(train_dataloader):
        ...
        ##########
        # # 保存mask1照片
        # print(out_net['x1_mask'])
        # print(out_net['x1_mask'].shape)
        # save_pic(out_net['x1_mask'], "x1_mask.png")
        # save_pic(out_net['x1'], "x1.png")
        # save_pic(out_net['x2'], "x2.png")
...
def main(argv):
    ...
    # # dataset_add
    # if args.dataset_add !="":
    #     train_dataset.add_datasets(args.dataset_add,split='train')
    #     test_dataset.add_datasets(args.dataset_add, split='test')
    ...
    if os.path.exists("checkpoint_best_loss.pth.tar"):
        ...
        # pretrained_dict = {k: v for k, v in model['state_dict'].items() if (k in new_dict and "_same_resolution" not in str(k))}#filter out unnecessary keys

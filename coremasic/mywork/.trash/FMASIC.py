...
class gmm_hyper_y1_same_resolution(nn.Module):
    def __init__(self,N,M,K): #K表示GMM对应正态分布个数
        ...
        # 已经同分辨率，不需要上采样
        self.gmm_sigma = nn.Sequential(
            deconv(in_channels=2*M,out_channels=4*M,kernel_size=3,stride=1), #,stride=2,padding=kernel_size//2,output_padding=stride-1
            nn.ReLU(inplace=True),

            deconv(in_channels=4*M, out_channels=4*M, kernel_size=3,stride=1),# stride=2,padding=kernel_size//2,output_padding=stride-1
            nn.ReLU(inplace=True),

            conv(in_channels=4*M,out_channels=(M*K),kernel_size=3,stride=1), #padding=kernel_size//2
            nn.ReLU(inplace=True),
        )

        self.gmm_means = nn.Sequential(
            deconv(in_channels=2*M, out_channels=4*M, kernel_size=3,stride=1),
            # stride=2,padding=kernel_size//2,output_padding=stride-1
            nn.LeakyReLU(inplace=True),

            deconv(in_channels=4*M, out_channels=4*M, kernel_size=3,stride=1),
            # stride=2,padding=kernel_size//2,output_padding=stride-1
            nn.LeakyReLU(inplace=True),
        ...
        self.gmm_weights = nn.Sequential(
            ...
            deconv(in_channels=4*M, out_channels=(M*K), kernel_size=3,stride=1),
            # # stride=2,padding=kernel_size//2,output_padding=stride-1
            # # nn.MaxPool2d(kernel_size=(H//16,W//16)), # ?? 换图像分辨率就要换模型了
            #
            ...
            conv(in_channels=(M*K), out_channels=(M * K), kernel_size=3, stride=1),  # padding=kernel_size//2
            #出去后要接一个softmax层，表示概率！！
        )
...
class gmm_hyper_y2_same_resolution(nn.Module):
    def __init__(self,N,M,K): #K表示GMM对应正态分布个数
        ...
        self.gmm_sigma = nn.Sequential(
            conv(in_channels=3*M,out_channels=6*M,kernel_size=3,stride=1),
            nn.ReLU(inplace=True),

            conv(in_channels=6*M,out_channels=6*M,kernel_size=3,stride=1),
            nn.ReLU(inplace=True),

            conv(in_channels=6*M,out_channels=(M*K),kernel_size=3,stride=1), #padding=kernel_size//2
            nn.ReLU(inplace=True),
        )
        ...
        self.gmm_weights = nn.Sequential(
            ...
            conv(in_channels=6*M,out_channels= (M*K),kernel_size=3,stride=1),
            # # nn.MaxPool2d(kernel_size=(H//16,W//16)), # ?? 换图像分辨率就要换模型了
            # spatial_pool2d(), #joint+gmm换1x1卷积不再需要
            nn.LeakyReLU(inplace=True),
...
class mask2weights(nn.Module):
    def __init__(self,N,M,Kw):
        ...
        self.maskconv = nn.Sequential(
            ...
            conv(in_channels=6, out_channels=Kw, kernel_size=3),  # stride = 2
            # 出去后接bottleneck
        )
        self.N = N
        self.M = M
...
class HSIC(CompressionModel):
    def __init__(self,N=128,M=192,K=5,**kwargs): #'cuda:0' or 'cpu'
        ...
        self.mask2weights_unit = mask2weights(N=N,M=M,Kw=2)
    ...
    def forward(self,x1,x2,h_matrix):
        ...
        gmm1_jg = self._h_s1_same_resolution(params1)  # 通道数 2M
        ###
        ...
        # }
        # print(x1.shape)
        # print(x1_mask.shape)#[1,3,512,512]
        ...
        ###########################
        gmm2_jq = self._h_s2_same_resolution(torch.cat((params2*mask_weights[:,0:1,:,:],y1_hat_warpf2*mask_weights[:,1:2,:,:]), dim=1)) #通道数 4M
        # gmm2_jq = self._h_s2_same_resolution(torch.cat((params2, y1_hat_warpf2),dim=1))  # 通道数 4M
        ###

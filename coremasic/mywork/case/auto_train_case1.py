...
for ii in lambda_list:
    print("cp ~/database/models/MASIC/"+database_name+"/"+"lambda_"+str(ii)+"/* ./")
    os.system("cp ~/database/models/MASIC/"+database_name+"/"+"lambda_"+str(ii)+"/* ./")

    os.system("""python newtrain_codec_real_case1.py -d "/home/ywz/database/"""+ database_name + """\"  --seed 0 --cuda """+str(cuda_id)+""" --patch-size """+ train_patch_size +""" --test-patch-size """+ resolution +""" --batch-size """+ train_batch_size +""" --test-batch-size 1  --save --lambda """+ str(ii) +""" -e 20""" + homopath)
    # os.system("""python newtrain_cqe_real.py -d "/home/ywz/database/"""+ database_name + """\"  --seed 0 --cuda """+str(cuda_id)+""" --patch-size """+ train_patch_size +""" --test-patch-size """+ resolution +""" --batch-size 1 --test-batch-size 1  --save --lambda """+ str(ii) +""" -e 25""" + homopath)

    os.system("""mkdir ~/database/models/MASIC_case1/"""+ database_name +"""/lambda_""" + str(ii) + """/""")
    #save
    os.system("""cp checkpoint_best_loss.pth.tar ~/database/models/MASIC_case1/"""+ database_name +"""/lambda_""" + str(ii) + """/""")

...
class Independent_EN(nn.Module):
    ...
    def forward(self, x1_hat, x2_hat, h_matrix):
        ...
        out1 = torch.cat((x2_hat_warp, x1_hat), dim=-3)
        out2 = torch.cat((x1_hat_warp, x2_hat), dim=-3)
        ...
        out1 = torch.cat((out1, out2_warp), dim=-3)
        out2 = torch.cat((out2, out1_warp), dim=-3)
        out1 = self.EBl2(out1)
        out2 = self.EBr2(out2)

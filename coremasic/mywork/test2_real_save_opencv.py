...
#python test3_real.py -d "/home/yangwenzhe/database/aftercut512"  --seed 0 --cuda 0 --patch-size 512 512 --batch-size 4 --test-batch-size 1  --save --lambda 0.01
...
from MASIC_save import *

out_root_path = "out_pic_aftercut512_opencv"
...
def test_epoch(epoch, test_dataloader, model, criterion):
    ...
    with torch.no_grad():
        for d in test_dataloader:
            ...
            save_pic(out_net['x1_warp'], osp.join(left_save_path, str(d[3]).split("'")[1]))
            save_pic(out_net['x2'], osp.join(right_save_path, str(d[3]).split("'")[1]))

...
def test_epoch(epoch, test_dataloader,modelhomo, model,model2, criterion):
    ...
    with torch.no_grad():
        for d in test_dataloader:
            ...
            input2 = torch.randn(1, 3, 512, 512).to(device)
            h_ = torch.randn(1, 3, 3).to(device)
            flops, params = profile(model, inputs=(input1, input2, h_))
            print(flops, params)
            ...
            input1 = torch.randn(1, 3, 512, 512).to(device)
            input2 = torch.randn(1, 3, 512, 512).to(device)
            flops, params = profile(model2, inputs=(input1, input2, h_))
            print(flops, params)
            flops, params = clever_format([flops, params], "%.3f")
            print(flops, params)

...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target1,target2,kind=0):
        ...
        if kind==0:
            ...
            out['lps1'] = loss_fn.forward(target1, output['x1_hat']).mean().item()
            out['lps2'] = loss_fn.forward(target2, output['x2_hat']).mean().item()
            out['lps'] = (out['lps1'] + out['lps2'])/2
...
def test_epoch(epoch, test_dataloader,modelhomo, model,model2, criterion):
    ...
    with torch.no_grad():
        for d in test_dataloader:
            ...
            lps_loss.update(out_criterion['lps'])  # 已除2
            lps_loss1.update(out_criterion['lps1'])  # 已除2
            lps_loss2.update(out_criterion['lps2'])  # 已除2
            ...
            # print(print_context)
            writeRow = [float(bpp_out_criterion['bpp1']),out_criterion['psnr1'],float(out_criterion['ms_ssim1']),out_criterion['lps1'],\
                float(bpp_out_criterion['bpp2']),out_criterion['psnr2'],float(out_criterion['ms_ssim2']),out_criterion['lps2']]
    ...
    print(f'Test epoch {epoch}: Average losses:'
          ...
          f'\tLPIPS: {lps_loss.avg:.4f} |'  # 已除2，相加时候便除了2
          f'\tLPIPS1: {lps_loss1.avg:.4f} |'
          f'\tLPIPS2: {lps_loss2.avg:.4f} |'
